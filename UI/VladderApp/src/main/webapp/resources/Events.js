class Controller{
   ...
   FilteredTags(){
    ...
    Sign(){
		...
		sendRequest('GET','user?name='+name)
			.then(function (data) {
				document.getElementById('myModal').style.display = "none";
				user = new User(name);
				var avatar = data.ava;
				document.getElementById('MainAvatar').src = data.ava;
			})
			.catch(function () {
                alert("Such user dont exist")
				user=new User("");
				document.getElementById('myModal').style.display="none";
			})
	...
	adding()
	{
        ...
        sendRequest('GET','add?name='+user.GetName()+'&Url='+url+'&description='+description+'&Tags='+f)
			.catch(function () {
alert("ERROR")
			})
			.then(function () {
	...
	function Edit() {
		...
		document.getElementById('Editing').addEventListener('click', function () {
			...
			let d;
			let f="";
			d=post.hashTags.split("#")
			for(var i=1;i<d.length;++i)
			{
				f=f.concat("%23")
				f=f.concat(d[i])
			}
			sendRequest('GET', 'edit?id=' + post.id + "&description=" + post.description + "&Url=" + post.photoLink
				+ "&tags=" + f)
				.catch(Alert("ERROR"))
				.then(function () {
	...
	function Delete()
		{
			sendRequest('GET','delete?id='+event.target.parentNode.parentNode.parentNode.id)
				.catch(function () {
                 alert("ERROR");
				})
				.then(function (data) {
				let localstorage = new PostsView(JSON.parse(localStorage.getItem('container')));
				localstorage.GetArrayPosts().remove(data);
				allPosts.DeletePost(data);
				localStorage.clear();
				localStorage.setItem("container", JSON.stringify(localstorage.GetArrayPosts().getPostsList()));
			})
		}
	function Like() {
		let localstorage = new PostsView(JSON.parse(localStorage.getItem('container')));
		sendRequest('GET', 'like?name=' + user.GetName() + '&id=' + event.target.parentNode.parentNode.parentNode.id)
			.catch(function () {
				alert("ERROR");
			})
