class PostForm {
	...
	<input id="Delete" type=image src="resources//Images//котэ-художник-мусорка-гифки-3892891.gif"></input>
...
let type = this._TypeOfPost.cloneNode(true);
        ...
        for(let i = 0; i < fillingOfPosts.length; i++){
            ...
            type.getElementsByClassName("date").item(0).textContent = filteredPosts[i].createdAt.getDate().toString() + "." + (filteredPosts[i].createdAt.getMonth()+1).toString() + "."
                + filteredPosts[i].createdAt.getFullYear().toString();
...
document.getElementById("help").parentNode.insertBefore(type,document.getElementById("help").nextSibling);
 ...
 post2= {
       id:1,
      description: 'Текст поста в котором всё логично всё ново всё необычно',
      createdAt: new Date(),
	  hashTags: ["#adfl","#dmks","#csdad"],
      author: 'Vladimir',
      likes: [],
...
post3= {
       id:2,
      description: 'туть бред написан не читай.',
      createdAt: new Date(),
	  hashTags: ["#adflk","#dmks","#csdad"],
      author: 'Vladic',
      likes: [],
      photoLink:''
    };
